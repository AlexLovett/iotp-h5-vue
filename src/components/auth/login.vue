  ...
  export default{
    ...
    created: function () {
      ...
      if (App.isLocal) {
        ...
        authUrl = process.env.LOCAL_HOST + ":8081";
      } else if (App.isDev) {
        accessHost = process.env.LOCAL_HOST + ":8000";
        authUrl = "http://192.168.199.10:8081";
      } else if (App.isStage) {
        accessHost = "http://test.xohaa.net";
      ...
      window.open(authUrl + "/oauth/authorize?client_id=" + appId + "&redirect_uri=" + encodeURI(accessHost), "_self");
    }
  }
</script>
